    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 form-group">
                        <label for="password">Contraseña <small>(Dejar en blanco para mantener la actual)</small></label>
                ...
                <div id="vet_fields" style="display: {{ old('rol', $user->rol) == 'veterinario' ? 'block' : 'none' }};">
                    <hr>
                    <h6 class="font-weight-bold text-info mb-3">Información del Veterinario</h6>
                    <div class="row">
                        <div class="col-md-4 form-group">
                            <label for="especialidad">Especialidad</label>
                            <input type="text" name="especialidad" class="form-control" id="especialidad" value="{{ old('especialidad', $user->veterinario->especialidad ?? '') }}">
                        </div>
                        <div class="col-md-4 form-group">
                            <label for="telefono">Teléfono</label>
                            <input type="text" name="telefono" class="form-control" id="telefono" value="{{ old('telefono', $user->veterinario->telefono ?? '') }}">
                        </div>
                        <div class="col-md-4 form-group">
                            <label for="cedula_profesional">Cédula Profesional</label>
                            <input type="text" name="cedula_profesional" class="form-control" id="cedula_profesional" value="{{ old('cedula_profesional', $user->veterinario->cedula_profesional ?? '') }}">
                        </div>
                    </div>
                </div>
                
                <div class="text-right mt-3">
                    <button type="submit" class="btn btn-primary px-5">Actualizar Usuario</button>
                </div>
            </form>
        </div>
    </div>
